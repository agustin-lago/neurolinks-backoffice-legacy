...
/**
 * Servicio para recargar fichas/saldo a un usuario en Cas-EPC utilizando Selenium.
 * 
...
export async function rechargeUserSelenium(
    ...
    console.log(`[Cas-EPC] Iniciando recarga de saldo de ${amount} para: ${username}...`);
    ...
    if (localDriver) {
        try {
            return await executeRecharge(localDriver, username, amount, shouldQuit, () => { hasClickedSubmit = true; });
        } catch (err: any) {
            console.error("❌ [Cas-EPC] Error en recarga con driver existente:", err.message);
            return false;
        }
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        console.log(`[Cas-EPC] Intento ${attempt}/${maxAttempts} para recarga (usuario: ${username})...`);
        ...
        if (proxySession) {
            console.log(`🔌 [Cas-EPC] Aplicando proxy a Chrome para recarga (Intento ${attempt}): ${proxySession.proxyUrl}`);
        ...
        try {
            ...
            const success = await executeRecharge(localDriver, username, amount, shouldQuit, () => { hasClickedSubmit = true; });
            if (success) {
                return true;
            } else {
                throw new Error("La ejecución de recarga retornó falso.");
            }
        } catch (error: any) {
            console.error(`❌ [Cas-EPC] Error en intento ${attempt} de recarga:`, error.message || error);
            ...
            if (localDriver) {
                try {
                    ...
                    const screenshotPath = path.join(process.cwd(), 'recharge_failure_casepc.png');
                    fs.writeFileSync(screenshotPath, screenshot, 'base64');
                } catch (e) {
                    // Ignorar error al guardar screenshot
                }
                try {
            ...
            // Si ya se presionó el botón de submit, no reintentamos para evitar duplicación de saldo
            if (hasClickedSubmit) {
                console.warn("⚠️ [Cas-EPC] Se aborta el reintento para evitar duplicación de saldo porque el formulario ya fue enviado.");
                return false;
            }
...
/**
 * Función interna que contiene la secuencia de navegación y ejecución de la recarga
 */
async function executeRecharge(
    ...
    try {
        ...
        // Ignorar si el campo no se pudo cambiar
    }
    ...
    try {
        ...
        // Ignorar si el botón no está disponible
    }
    ...
    // 4. Ingresar el monto en el input de depósito de la fila correspondiente
    ...
    // 5. Clic en el botón de depósito (button[1])
    const submitDepositBtnXPath = "/html/body/div[7]/div[2]/div/div[2]/div[2]/div[3]/div[2]/div[2]/table/tbody/tr[1]/td[5]/div/form/button[1]";
        ...
        for (const el of errorElements) {
            try {
                if (await el.isDisplayed()) {
                    ...
                    if (text && text.trim() !== '') {
                        console.error(`❌ [Cas-EPC] Error al realizar depósito: "${text}"`);
    ...
    console.log(`🎉 [Cas-EPC] Recarga de saldo completada con éxito para ${username}.`);
