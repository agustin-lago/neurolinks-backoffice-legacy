...
export async function createUserSelenium(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        console.log(`[Cas-EPC] Intento ${attempt}/${maxAttempts} para crear usuario (baseName: ${baseName})...`);
        ...
        if (proxySession) {
            console.log(`🔌 [Cas-EPC] Aplicando proxy a Chrome (Intento ${attempt}): ${proxySession.proxyUrl}`);
        ...
        try {
            ...
            if (result && !result.success) {
                throw new Error(result.error || "Fallo en el proceso de creación");
            }

            console.log(`🎉 [Cas-EPC] Jugador creado con éxito: ${usernameGenerated}`);

            // Lógica condicional del boolean recharge
            if (recharge) {
